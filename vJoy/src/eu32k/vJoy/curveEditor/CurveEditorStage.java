...
public class CurveEditorStage extends Stage {
   ...
   private Rectangle zoomRange = new Rectangle(0.03f, 0.03f, 1.0f, 1.0f);

   public CurveEditorStage(String filePath) {

      track = new AudioTrack(Gdx.files.absolute(filePath), false);

      device = Gdx.audio.newAudioDevice(track.getRate(), track.getChannels() == 1);
      player = new MusicPlayer(device, track);
      pixmapChannel1 = new SpectrumPixmap(1920, 1080, track, true);
      // pixmapChannel2 = new SpectrumPixmap(1920, 1080, samplesChannel2, true);
      ...
      table.row();
      table.add(new PixmapWidget(pixmapChannel1)).colspan(2).fill().expand().pad(5);
      // table.row();
   ...
   public void draw() {
      ...
      if (lastPosition != player.getPosition() || lastZoomX != zoomX || lastZoomY != zoomY) {
         ...
         float fromX = (float) player.getPosition() / track.getAllSamples().length - zoomX / 2.0f;
